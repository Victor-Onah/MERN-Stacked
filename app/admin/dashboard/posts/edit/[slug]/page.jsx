...
export default function Page({ params }) {
	...
	useLayoutEffect(() => {
		document.title = "Admin | Edit Post";
		(async () => {
			try {
				...
				if (status === "successful") {
					...
					document.title = `Admin | Edit Post: ${data.title}`;
				}
			} catch (error) {
	...
	return (
		<>
			<div className="max-w-screen-xl mx-auto px-4 pt-6 pb-4">
				{postLoadStatus === "pending" && <LoadingUI />}
				{postLoadStatus === "failed" && <ErrorUI />}
				{postLoadStatus === "successful" && (
					<>
						<h1 className="text-4xl lg:text-6xl md:text-5xl font-semibold">
							Edit post
						</h1>
						<p className="p-1 px-2 bg-zinc-100 rounded-full text-sm w-fit mt-2">
							{title}
						</p>
